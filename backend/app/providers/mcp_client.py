# 文件：backend/app/providers/mcp_client.py
# 作用：MCP 工具接入：按 config/mcp.json 起 stdio server → 列工具 → 白名单内调用；返回内容按 §7 当不可信数据包裹
# 阶段：P8 MCP 插件接入
# 依赖：标准库 asyncio、json、os、sys；mcp 官方 SDK；backend/app/core/config.py、
#       backend/app/services/store.py、backend/app/providers/kb.py（共用指令行剥离）
