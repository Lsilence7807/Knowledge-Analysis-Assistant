...
> **本文写的是完工形态**：设计文档 §5 的 P0~P24 全部段落（桌面端 P12 已移出范围），能力、接口、数据模型与验收口径都以 `docs/系统总体设计.md` 的 §4 冻结契约为准。逐段实现进度、已落地文件与已知问题上限见 `docs/代码台账.md`，问题清单见 `docs/问题总表.md`。
...
- SQL 只放行单条只读 SELECT，越权、超时、超行数一律拒绝；**跨表 join 必须命中已确认的关系**（P22），不让模型猜连接条件；
...
**已实现**：MVP 七段（P0 骨架 → P1 摄入 → P2 SQL 与统计 → P3 单句提问 → P4 结论 → P5 前端 → P13 Agent）+ P6 技能 + P7 知识库 + P8 MCP + P15 沙箱与指标 + P17 流式 + P21 性能并发契约 + P5.1/P5.2/P5.3 视觉与一键启动；P9 多模型 / P10 记忆与缓存 / P11 部署与登录 / P14 语义检索 / P16 评测与观测 / P18 作业 / P19 文档与报告 已随框架化改造落地。

**框架化改造（F1~F10，已完成）**：自研实现逐段换成现成框架——LiteLLM（模型层）→ LangGraph + SqliteSaver（Agent 与记忆）→ LlamaIndex + LanceDB（检索与向量）→ SQLAlchemy + Alembic + pydantic-settings（数据层）→ sqlglot + RestrictedPython + itsdangerous + slowapi（守卫与认证）→ Langfuse + deepeval（观测与评测）→ huey + APScheduler + MarkItDown（作业与文档）→ 多阶段镜像 + compose + Caddy（部署）。十段换完，接口与既有断言零改动。

**未开工**（设计已定，按 `docs/系统总体设计.md` §5 施工）：

- P20 日常使用功能：收藏问题一键重跑、数据集重导入与 schema drift 报告、同结构双表对比、结果导出 CSV/Excel
- P22 多表关系推断与 join 校验 · P23 企业数据源只读同步 · P24 反馈闭环与成本配额
- 小块留白：检索侧的 rerank 与 query 改写、评测侧的 Rubrics 三维与赞踩回流、成本看板页

**明确不做**：模型微调 / LoRA / 蒸馏 / 自训练；K8s、多租户 RBAC、合规审计；自建推理集群与向量引擎、embedding；自研 AI 网关、计费与训练平台；移动端适配；扫描件 OCR；对企业库的写操作与 DDL；自研 ETL 调度平台；桌面端安装包（P12，2026-09-21 移出范围）。
